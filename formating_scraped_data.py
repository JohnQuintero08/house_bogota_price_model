...
def transform_detail_to_status_age(raw_detail, loc):
    try:
        data = raw_detail[loc]
        if data == '¡Pregúntale!':
            return np.nan
...
def feature_columns(df):
    ...
    new_df['stratum'] = new_df['details'].apply(lambda x: transform_detail_to(x, 0))
    new_df['type'] = new_df['details'].apply(lambda x: transform_detail_to(x, 1))
    new_df['status'] = new_df['details'].apply(lambda x: transform_detail_to_status_age(x, 2))
    new_df['bathrooms'] = new_df['details'].apply(lambda x: transform_detail_to(x, 3))
    ...
    new_df['age'] = new_df['details'].apply(lambda x: transform_detail_to_status_age(x, 6))
    new_df['rooms'] = new_df['details'].apply(lambda x: transform_detail_to(x, 7))
